...
// Helper interface for multer request
// interface RequestWithFile removed as it is unused
  ...
  try {
    ...
    if (
      ...
      await new Promise<void>((resolve, reject) => {
        ...
        upload.any()(request as Request, response, (error: unknown) => {
          if (error) {
    ...
    if (validatedData.is_publish !== undefined) {
      ...
      updateFields.is_published =
        (isPublishValue as unknown) === 'true' ||
        isPublishValue === true ||
        (isPublishValue as unknown) === '1';
